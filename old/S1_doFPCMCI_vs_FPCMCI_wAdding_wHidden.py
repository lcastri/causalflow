...
def save_result(startTime, stopTime, scm, alg, gt):
    res_tmp[alg][sta._TIME] = str(stopTime - startTime)
    res_tmp[alg][sta._PREC] = sta.precision(gt, cm = scm)
    res_tmp[alg][sta._RECA] = sta.recall(gt, cm = scm)
    res_tmp[alg][sta._F1SCORE] = sta.f1_score(res_tmp[alg][sta._PREC], res_tmp[alg][sta._RECA])
    res_tmp[alg][sta._SHD] = sta.shd(gt, cm = scm)
...
if __name__ == '__main__':   
    ...
    resfolder = 'S1_doFPCMCI_vs_FPCMCI_wAdding_wHidden'
    Tobs = int(T*P_OBS)
    Tint = int(T*P_INT)
    ...
    for n in nfeature:
        ...
        for nc in range(ncoeff):
            num_int = random.randint(1, min(n, MAX_N_INT - MAX_N_HID))
            ...
            save_result(startFPCMCI, stopFPCMCI, fpcmci_scm, sta._FPCMCI, gt_n_hidden)
            ...
            dofpcmci_scm = scm
            save_result(start_doFPCMCI, stop_doFPCMCI, dofpcmci_scm, sta._doFPCMCI, gt_n_hidden)
