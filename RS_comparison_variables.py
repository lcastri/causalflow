...
def save_result(fpcmci_t, fpcmci_scm, dofpcmci_t, dofpcmci_scm):
    print("\n")
    ...
    for algo, scm, t in zip([sta._FPCMCI, sta._doFPCMCI], [fpcmci_scm, dofpcmci_scm], [fpcmci_t, dofpcmci_t]):
...
if __name__ == '__main__':   
    resdir = "multi_conf_nonlin_1000_1000_0_0.5"
    f_alpha = 0.05
    pcmci_alpha = 0.05
    min_lag = 1
    max_lag = 2
    ...
    for n in nfeature:
        for nr in range(nrun):
            ...
            while True:
                try:
                    ...
                    RS = RandomSystem(nvars = n, nsamples = nsample, 
                                        ...
                                        functions = [''], operators=['+', '-', '*'], n_hidden_confounders = random.randint(1, 2))
                    RS.gen_equations()
            ...
            #########################################################################################################################
            # SAVE
            save_result(fpcmci_time, get_correct_SCM(GT, fpcmci_cm.get_SCM()),
