...
for bagname in BAGNAME:
    for wp in [WP.DELIVERY_POINT]:
        for tod in TOD:
            if tod == TOD.STARTING:
                starting = pd.read_csv(os.path.join(INDIR, "my_nonoise", f"{bagname}", tod.value, f"{bagname}_{tod.value}_{wp.value}.csv"))
            dfs.append(pd.read_csv(os.path.join(INDIR, "my_nonoise", f"{bagname}", tod.value, f"{bagname}_{tod.value}_{wp.value}.csv")))
...
num_ticks = 25
tick_indices = np.linspace(0, len(T) - 1, num_ticks, dtype=int)
tick_labels = [time.strftime("%H:%M:%S", time.gmtime(T[idx])) for idx in tick_indices]
