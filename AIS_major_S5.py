...
def get_ambiguous_link(scm):
    ...
    return amb_links, potentialIntervention
...
def fill_res(r):
    ...
    ambiguous_links, _ = get_ambiguous_link(r["graph"])
        ...
        while True:
            try:
                ...
                if nr in data and data[nr]['done']: break
                    ...
                    amb_links, _ = get_ambiguous_link(GT_GRAPH)
                ...
                if Algo.LPCMCI.value not in data[nr] or (Algo.LPCMCI.value in data[nr] and not data[nr][Algo.LPCMCI.value]['done']):
                    ...
                    try:
                        ...
                        _, potentialIntervention = get_ambiguous_link(lpcmci_cm.get_Graph())
                        
                        if len(potentialIntervention) < ninterventions: 
                            gc.collect()
                            remove_directory(os.getcwd() + '/' + resfolder)
                            continue
                        else:
