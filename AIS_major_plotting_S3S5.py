...
def extract_data(file_path, algorithm, n, metric, mode = ExtractDataMode.MeandStd, candoit_mode = 'best'):
    ...
    with open(file_path) as json_file:
        ...
        for i in r.keys():
            ...
            if metric == Metric.TIME:
                ...
                if algorithm == Algo.CAnDOIT.value:
                    if candoit_mode == 'best': 
                        ext_data[algorithm]["samples"].append((10*math.log(r[i][str(n)][selected_candoit][metric.value], 10)))
                    if candoit_mode == 'mean':
                        tmp = np.mean([10*math.log(r[i][str(n)][candoit_run][metric.value], 10) for candoit_run in candoit_keys])
                        ext_data[algorithm]["samples"].append((tmp))
                else:
    ...
    for n in range(nvars[0],nvars[1]+1):    
        for algo in toPlot.keys():
            if algo.startswith('candoit'):
                mode = algo.split('_')[1]
                ext_data = extract_data(res_path, 'candoit', n, metric, mode = ExtractDataMode.MeandStd if not bootStrap else ExtractDataMode.BootStrap, candoit_mode=mode)
                a = 'candoit'
            else:
                ext_data = extract_data(res_path, algo, n, metric, mode = ExtractDataMode.MeandStd if not bootStrap else ExtractDataMode.BootStrap)
                a = algo
...
if __name__ == '__main__':      
    ...
    for metric in metrics:
        ...
        compare(resfolder, algorithms, metric, ninterventions, plot_style, plotType.LinewErrorBar, bootStrap = bootstrap)