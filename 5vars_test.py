...
# for t in range(1, T):
#     B[t] = 0.5 * A[t] + 0.3 * E[t-1] + np.random.normal(0, 0.1)
#     C[t] = 0.7 * B[t] + np.random.normal(0, 0.1)
#     D[t] = 0.6 * C[t] + 0.4 * E[t-1] + np.random.normal(0, 0.1)
...
smc = SMCFilter(cie.DBNs[('obs', 0)], num_particles=100)
...
with pm.Model() as model:
    ...
    B_t = pm.Normal("B", mu=0.5 * A_t + 0.3 * E_t, sigma=sigma_B, shape=T)
    C_t = pm.Normal("C", mu=0.7 * B_t, sigma=sigma_C, shape=T)
    D_t = pm.Normal("D", mu=0.6 * C_t + 0.4 * E_t, sigma=sigma_D, shape=T)
