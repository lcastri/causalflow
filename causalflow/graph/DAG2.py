...
class DAG():
    ...
    #                            node_size = node_size,
    #                            standard_color_nodes = node_color if isinstance(node_color, str) else None,
    #                            special_nodes = node_color if isinstance(node_color, dict) else None,
    #                            cmap_edges = None,
    #                            standard_color_links = 'grey',
    ...
    def get_val_matrix(self) -> np.array:
        ...
        for t in self.g.keys():
            for s, info in self.g[t].sources.items():
                    ...
                    # if s[1] == 0: r[self.features.index(s[0]), self.features.index(t)][0] = info[SCORE]
        # if not self.isSymmetric(r[:,:,0]):
        #     raise ValueError("non-symmetric val_matrix at lag 0")
        return np.array(r)
    ...
    def get_graph_matrix(self) -> np.array:
        ...
        for t in self.g.keys():
            for s, info in self.g[t].sources.items():
                ...
                # FIXME: to be tested
                # if r[self.features.index(t), self.features.index(s[0])][s[1]] == '':
                #     r[self.features.index(t), self.features.index(s[0])][s[1]] = info[TYPE]
                #     r[self.features.index(s[0]), self.features.index(t)][s[1]] = self.__opposite_arrow(info[TYPE])
        return np.array(r)
    ...
    # @staticmethod
    # def __opposite_arrow(arrow):
    #     if arrow == LinkType.Directed.value:
    #         return '<--'
    #     elif arrow == LinkType.Uncertain.value or arrow == LinkType.Bidirected.value:
    #         return arrow
    #     elif arrow == LinkType.HalfUncertain.value:
    #         return '<-o'
