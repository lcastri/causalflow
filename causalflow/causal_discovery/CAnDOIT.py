...
class CAnDOIT(CausalDiscoveryMethod):
    ...
    def run(self, remove_unneeded = True, nofilter = False) -> DAG:
        ...
        if not self.isThereInterv:
            ...
            self.CM = fpcmci.run(remove_unneeded=remove_unneeded, nofilter=nofilter)
            ...
            if not nofilter:
                ...
                # Create a dummy fully-connected DAG to build the link_assumptions dict
                cm = DAG(self.data.features, self.min_lag, self.max_lag)
                cm.fully_connected_dag()
                cm.sys_context = self.CM.sys_context
                cm.add_context() 
                link_assumptions = cm.get_link_assumptions()
    ...
    def _prepare_data(self, obser_data, inter_data, plot_data):
        ...
        # FIXME: this returns an error when the plot is closed
