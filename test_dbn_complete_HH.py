...
for f in FEATURES:
    ...
    observation = np.concatenate((DATA_DICT_TRAIN.d.values[-CM.max_lag:, i].reshape(-1, 1), DATA_DICT_TEST.d.values[0,i].reshape(-1, 1), np.nan*np.ones_like(res[:, i]).reshape(-1, 1)), axis=0).reshape(-1)
    prediction_f = np.concatenate((np.nan*np.ones_like(DATA_DICT_TRAIN.d.values[-CM.max_lag:, i]), res[:, i]), axis=0)
    ...
    if f in [NODES.RB.value, NODES.BAC.value]: 
        ...
        observation = np.concatenate((obs_1, obs_2, np.nan*np.ones_like(res[:, i]).reshape(-1, 1)), axis=0)
        prediction_f = np.concatenate((np.nan*np.ones_like(DATA_DICT_TRAIN.d.values[-CM.max_lag:, i]), np.mean(res[:, i]) * np.ones_like(res[:, i])), axis=0)
