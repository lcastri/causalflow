...
for f in FEATURES:
    ...
    if f in [NODES.RB.value, NODES.BAC.value]: 
        observation = np.floor(np.concatenate((DATA_DICT_TRAIN.d.values[-CM.max_lag:], DATA_DICT_TEST.d.values[0,:].reshape(1,-1), np.nan*np.ones_like(res)), axis=0))
        prediction_f = np.floor(np.concatenate((np.nan*np.ones_like(DATA_DICT_TRAIN.d.values[-CM.max_lag:]), res), axis=0))
