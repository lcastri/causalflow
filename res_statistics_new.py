...
def compare(resfolder, algorithms, metric, nvars, plotStyle, plot_type = plotType.LinewErrorBar, bootStrap = False, show = False, xLabel = '# vars'):
    ...
    if plot_type != plotType.BoxPlot:
        ...
        if plot_type == plotType.Line:
            for algo in algorithms:
                plt.plot(range(nvars[0], nvars[1]+1), toPlot[algo.value]["means"], 
                        marker=plotStyle[algo.value]['marker'], color = plotStyle[algo.value]['color'], linestyle = plotStyle[algo.value]['linestyle'])
            ...
            for algo in algorithms:
                plt.errorbar(range(nvars[0], nvars[1]+1), toPlot[algo.value]["means"], toPlot[algo.value]["confidences"], 
                             marker=plotStyle[algo.value]['marker'], capsize = 5, color = plotStyle[algo.value]['color'], linestyle = plotStyle[algo.value]['linestyle'])
        ...
        plt.xlabel(xLabel)
        plt.ylabel(plotLabel[metric.value])
        plt.legend([plotLabel[algo.value] for algo in algorithms], loc='best')
        plt.grid()
        plt.title(titleLabel[metric.value] + ' comparison')
    ...
    if show:
        ...
        plt.savefig(os.getcwd() + "/results/" + resfolder + "/" + metric + '.pdf')
        plt.savefig(os.getcwd() + "/results/" + resfolder + "/" + metric + '.png')
