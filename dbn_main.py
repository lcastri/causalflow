...
T = 3000
N = 5
d = np.random.normal(0, 1, size = (T, N))
for t in range(max_lag, T):
    d[t, 1] += 0.75 * d[t-1, 0]**2
    d[t, 2] += 1.3 * d[t-1, 0] * d[t-2, 1]
    d[t, 3] += 0.7 * d[t-1, 3] * d[t-2, 4]
    
train_perc = 0.8
train_len = int(T*train_perc)

df_train = Data(d[:train_len])
df_test = d[train_len:]

# df_train.plot_timeseries()

fpcmci = FPCMCI(df_train, 
                f_alpha = alpha, 
                alpha = alpha, 
                min_lag = min_lag, 
                max_lag = max_lag, 
